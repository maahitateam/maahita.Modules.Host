<script lang="ts">
	...
	import { apihost, appModules, getAuthHeaders, getDateParts, hasKey } from '../../appconfig';
	import { presenters, presenter_sessions, isAdmin, token, current_presenter } from '../../store';
	...
	const handleAddSession = async (event: CustomEvent) => {
		try {
			...
			if (sessiondata.isadd) {
				...
				$presenter_sessions = temp
					...
					.sort(
						(a, b) => new Date(b['scheduledon']).valueOf() - new Date(a['scheduledon']).valueOf()
					);
	...
	const handleAddPresenter = async (event: CustomEvent) => {
		try {
	...
	const closeAddPresenter = async (event: CustomEvent) => {
		try {
	...
	const closeaddsession = async (event: CustomEvent) => {
		const close = event.detail;
		if (close) {
			module_config.show_add_session = false;
		}
	...
	const handlecanelsession = async (event: CustomEvent) => {
	...
	const handlestartsession = async (event: CustomEvent) => {
		const data = event.detail;
		notifyConfig = {
			cssClass: 'is-info',
	...
	const handlestopsession = async (event: CustomEvent) => {
		const data = event.detail;
		notifyConfig = {
			cssClass: 'is-info',
	...
	const handledeletesession = (event: CustomEvent) => {
		const data = event.detail;
...
{#if $isAdmin}
	...
	{:else}
		<div class="columns" style="padding-left: 2rem;padding-right: 2rem;padding-top:2rem;">
