<script lang="ts">
	...
	import { apihost, appModules, getAuthHeaders, hasKey, getDateParts } from '../../appconfig';
	import {
		loggedInUserId,
		presenter_sessions,
		token,
	...
	const handleAddPresenter = async (event) => {
		try {
			const presenter = event.detail as IPresenter; // dispatch event data is avaible in event.detail
	...
	const handledeletesession = (event) => {
		...
		$presenter_sessions = $presenter_sessions.filter((s) => s.id !== session.id);
	};
	const submitrequest = (event: CustomEvent) => {
		const data = event.detail;
		notifyConfig = {
			...notifyConfig,
