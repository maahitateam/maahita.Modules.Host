<script lang="ts">
	...
	import { apihost, appModules, getAuthHeaders, getFormattedDate } from '../../appconfig';
	...
	import { ISessionRequest } from 'src/interfaces';
	$: notifyConfig = {
		cssClass: 'is-info',
		message: ''
	};
	...
	onMount(async () => {
		if ($isAdmin) {
			...
			result.forEach((d: ISessionRequest) => {
	...
	const approvesession = async (session: ISessionRequest) => {
		try {
			...
			if (result) {
				$sessionrequests = $sessionrequests.filter((ses) => ses.id !== session.id);
	...
	const rejectsession = async (session: ISessionRequest) => {
		try {
			const url = `${apihost}/${appModules.sessionrequest}/${session.id}`;
			const headers = getAuthHeaders($token);
				...
				<div class="box">
					{#each $sessionrequests as session}
						<article class="media">
							...
							<div class="column schedule">{session.scheduledon}</div>
							<div class="column schedule">{session.description}</div>
							<div class="column presenter">{session.presentername}</div>
							<div class="column buttons">
								<button class="button is-primary" on:click={() => approvesession(session)}>
									Approve
								</button>
								<button class="button is-danger" on:click={() => rejectsession(session)}>
									Reject
								</button>
							</div>
						</article>
