<script lang="ts">
	...
	import { apihost, appModules, getAuthHeaders, hasKey } from '../../appconfig';
	...
	<article class="media">
		...
		<div class="media-content">
			...
			{#if role === 'user'}
				<div class="buttons">
					...
					{:else}
						<button on:click={handleEnroll} type="button" class="button is-text"> Enroll </button>
					{/if}
					{#if action_status === 3 && data['isEnrolled']}
						<button type="button" class="button is-info is-inverted">
			...
			{:else}
				<div class="buttons">
					<button on:click={handleEditSession} class="button is-primary is-active">
						<span class="icon"> <i class="far fa-edit" /> </span>
						<span>Edit</span>
					</button>
					{#if action_status === 1}
						<button type="button" on:click={handleStartSession} class="button is-link is-active">
							<span class="icon"> <i class="fas fa-play" /> </span>
							<span>Start</span>
						</button>
					{/if}
					{#if action_status === 3}
						<button type="button" on:click={handleStopSession} class="button is-link is-active">
							<span class="icon"> <i class="fas fa-stop-circle" /> </span>
							<span>Complete</span>
						</button>
					{/if}
					<button type="button" on:click={handleCancelSession} class="button is-warning is-active">
						<span class="icon"> <i class="fas fa-window-close" /> </span>
						<span>Cancel</span>
					</button>
					<button type="button" on:click={handleDeleteSession} class="button is-danger">
						<span class="icon"> <i class="fas fa-trash" /> </span>
						<span>Delete</span>
					</button>
					{#if !data['notification']}
						<button type="button" class="button is-success" on:click={() => sendNotification(data)}>
		...
		{#if role === 'user'}
			<div class="media-right">
				{#if data['photoURL']}
					<figure class="figure">
						<img src={data['photoURL']} class="profile" alt="profile place holder" />
					</figure>
				{:else}
